...
public class BC {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < Integer.parseInt(args[2]); i++)
		{
			...
			while(count < 5)
			{
				...
				conf3.set("start", starts);

				Job job3 = Job.getInstance(conf3, "Job 3");
				...
				Job job4 = Job.getInstance(conf4, "Job 4");
			...
			Job job5 = Job.getInstance(conf5, "Job 5");
			...
			Job job6 = Job.getInstance(conf6, "Job 6");
	...
	static class Map1 extends Mapper<LongWritable, Text, Text, Text>{
		...
		protected void map(LongWritable key, Text value, Context context) throws java.io.IOException, InterruptedException {
			...
			{
				context.write(new Text(vertices[0]), new Text(vertices[0] + "," + vertices[1]));
				context.write(new Text(vertices[1]), new Text(vertices[0] + "," + vertices[1]));
			}
			else if(vertices[0].compareTo(vertices[1]) > 0)
			{
				context.write(new Text(vertices[0]), new Text(vertices[1] + "," + vertices[0]));
				context.write(new Text(vertices[1]), new Text(vertices[1] +	"," + vertices[0]));
			}
		}
	}
	...
	static class Map3 extends Mapper<LongWritable, Text, Text, Text>{
		...
		protected void map(LongWritable key, Text value, Context context) throws java.io.IOException, InterruptedException {
			...
			String[] valueSplit = value.toString().split("\t");
			String[] vertices = valueSplit[0].split(",");
			context.write(new Text(vertices[0]), new Text(valueSplit[1]));
			context.write(new Text(vertices[1]), new Text(valueSplit[1]));
		}
	}
		...
		protected void reduce(Text key, Iterable<Text> values, Context context) throws java.io.IOException, InterruptedException {
			...
			for (int i = 0; i < starts; i++) {
				...
				for (int j = 0; j < d; j++) {
					...
					newPositions += (1.0 / (d + 1)) * Double.parseDouble(positions[j][otherVertexIndex[j]][i]);

				}
				// We also need to add (1/(d + 1))*a to the new position, where
				// a is the current position value of keyVertex
				newPositions += (1.0 / (d + 1)) * Double.parseDouble(positions[0][1 - otherVertexIndex[0]][i]);
			...
			for (int i = 0; i < d; i++) {
				...
				context.write(new Text(vertices[i][0] + "," + vertices[i][1]), new Text(keyVertex + "-" + newPositionsVector));
			}
		}
	}
	...
	*/
	...
	static class Map5 extends Mapper<LongWritable, Text, Text ,Text> {
		...
		protected void map(LongWritable key, Text value, Context context) throws java.io.IOException, InterruptedException {
			...
			String ver2 = val[0].substring(idx1+1);
	...
	static class Reduce6 extends Reducer<Text,Text,Text,Text>{
		...
		protected void reduce(Text key, Iterable<Text> values, Context context) throws java.io.IOException, InterruptedException {
			...
			if(avg <= (aPrime))
			{
				...
				context.write(new Text(key.toString().substring(0,idx)), new Text(key.toString().substring(idx+1)));
			}
		}
	}
}
